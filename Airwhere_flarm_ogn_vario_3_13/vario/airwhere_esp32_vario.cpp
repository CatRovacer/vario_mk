...
//#define IMU_DEBUG
//#include "airwhere_esp32_vario.h"
...
// this is done by commands, the other on buzzer is for when you switch on .
...
//setup serial port for write to bluetooth module
//HardwareSerial BT(2);


//char LK8EX1_Sentence[80];
//char GPS_Sentence[80];

#define PRESSURE_AT_18000_FT 50000
// this needs to be the lowest pressure we find - on high pressure days its less than 1013 millibars
// 110000 is 700m below sea level on a standard pressure day so should be ok.
#define PRESSURE_AT_0_FT 110000
...
volatile int last_sample_pressure;
...
int net_vario = 10; //default to 1s integrated vario; net vari is in 0.1s increments



int32_t audioCps;

// switched to use main loops channel int audiochannel = 1;
int audiochannel = 0;
...
//int pinSCL = 35;
int pinDRDYInt = 12;
// change to the one in the main loop int pinAudio = 13;
int pinAudio = 32;
...
static volatile int drdyCounter;
static volatile int drdyFlag;
int baroCounter = 0;;
int lk8exCounter = 0;
...
void updateTime() {
	timeNowUs = micros();
	imuTimeDeltaSecs = timeNowUs > timePreviousUs ? (float)(timeNowUs - timePreviousUs) : 2000.0f; // if rollover use expected time difference
	imuTimeDeltaSecs = imuTimeDeltaSecs / 1000000.0f;
...
float runningAverageofCps(float cps)
{
	...
	if (variosamplecnt > (net_vario * 50)/10) variosamplecnt = (net_vario * 50/10);
...
void vario_setup() {
	...
	//Wire.setClock(400000); // set clock frequency AFTER Wire.begin()
     ...
     Wire.setClock(200000); // set clock frequency AFTER Wire.begin()
	//5.4
	Wire.setTimeOut(250);



	audio.Config(pinAudio, audiochannel,sink_threshold,climb_threshold, zero_threshold);
	...
	if (preferences.begin("fastvario", false))
	{
	...
	else
	{
		...
		gy_offset=0;
		gz_offset = 0;
		kfzVariance=KF_ZPRESS_VARIANCE;
		kazVariance=KF_ZACCEL_VARIANCE;
	}
...
*/
...
#ifdef dmp
	// make sure it dmp initialisation worked (returns 0 if so)
	if (devStatus == 0) {
...
#endif
	//read the config value for vario calibration now instead of a button
	//Serial.println("calibrate value "); Serial.println(calibrate_vario);
	if (calibrate_vario == 'y') {
		// acknowledge calibration long tone
	...
	//calibrate pressure sensor always
	...
	baroCounter = 0;
	timeoutSeconds = 0;
	Serial.printf("Finshed vario setup\r\n");
...
void vario_loop() {
	...
	//Serial.println("vario loop");
	while (run_vario)
	{
		//v5
		audio.UpdateConfig(sink_threshold,climb_threshold, zero_threshold);

#ifdef dmp
		// wait for MPU interrupt or extra packet(s) available

	// goes into here and locks - ?
	...
	//		Serial.println("vario loop 1b");
		...
		if (drdyFlag) { // new MPU6050 data ready, 100Hz ODR => ~10mS sample interval
			...
			if (imuTimeDeltaSecs < 0.009 || imuTimeDeltaSecs > 0.011)
			{
				//taking too long due to config settings being chnaged for example so lets ignore and reset
				baro.InitializeSampleStateMachine();
				baroCounter = 0;
				zAccelAccumulator = 0.0f;
				baroTimeDeltaSecs = 0.0f;
			...
			{
#ifdef dmp
				//compensated acceleration in cm/s/s
				float gravityCompensatedAccel = 980.0 * float(aaWorld.z*(1.0f / MPU6050_2G_SENSITIVITY));
				//compensate for high banked turns if g force > +- 0.5g ignore
				if (abs(gravityCompensatedAccel) > (980 / 2.0)) {
					gravityCompensatedAccel = 0.0;

				}
				if (debug_level > 1) {
					Serial.print("gravity compensated acceleration: ");
					Serial.println(gravityCompensatedAccel);
				}
#else
				// accelerometer samples (ax,ay,az) in Gs, gyroscope samples (gx,gy,gz) in deg/second
				//imu.GetAccelGyroData(accel, gyro);
				mpu.getMotion6(&ax, &ay, &az, &gx, &gy, &gz);
				accel[0] = float(ax - ax_offset)*(1.0f / MPU6050_2G_SENSITIVITY);
				accel[1] = float(ay - ay_offset)*(1.0f / MPU6050_2G_SENSITIVITY);
				accel[2] = float(az - az_offset)*(1.0f / MPU6050_2G_SENSITIVITY);
				gyro[0] = float(gx - gx_offset)*(1.0f / MPU6050_500DPS_SENSITIVITY);
				gyro[1] = float(gy - gy_offset)*(1.0f / MPU6050_500DPS_SENSITIVITY);
				gyro[2] = float(gz - gz_offset)*(1.0f / MPU6050_500DPS_SENSITIVITY);

				//only use acceleration in calculation if g force between 0.75G and 1.25G
				float accelMagnitudeSquared = accel[0] * accel[0] + accel[1] * accel[1] + accel[2] * accel[2];

				int bUseAccel = ((accelMagnitudeSquared > 0.56250f) && (accelMagnitudeSquared < 1.5625f)) ? 1 : 0;
				...
				if (abs(gravityCompensatedAccel) > (980 / 2.0)) {
					//Serial.print("gravity compensated acceleration before setting to 0: "); Serial.println(gravityCompensatedAccel);
					gravityCompensatedAccel = 0.0;

				}
				if (debug_level > 1) {
					Serial.print("gravity compensated acceleration: ");
					Serial.print(gravityCompensatedAccel);
					Serial.print(",");
					Serial.println(gravityCompensatedAccel / 980.0);
				}
...
#ifdef OUTPUT_TEAPOT
				// display quaternion values in InvenSense Teapot demo format:
				teapotPacket[2] = int(q0*16384.0)>>8;
				teapotPacket[3] = int(q0*16384.0) && 0xFF;
				teapotPacket[4] = int(q1*16384.0) >> 8;
				teapotPacket[5] = int(q1*16384.0) && 0xFF;
				teapotPacket[6] = int(q2*16384.0) >> 8;
				teapotPacket[7] = int(q2*16384.0) && 0xFF;
				teapotPacket[8] = int(q3*16384.0) >> 8;
				teapotPacket[9] = int(q3*16384.0) && 0xFF;
				Serial.write(teapotPacket, 14);
				teapotPacket[11]++; // packetCount, loops at 0xFF on purpose
#endif
#endif
				...
				if (baroCounter >= 1) { // ~10mS elapsed, this is the sampling period for MS5611,
					...
					if (zMeasurementAvailable) {
						...
						//Serial.print("baro in pascal "); Serial.println(baro.paSample_); Serial.print("last pressure reading "); Serial.println(last_sample_pressure);

						if (int(baro.paSample_) > PRESSURE_AT_18000_FT && int(baro.paSample_) < PRESSURE_AT_0_FT && (last_sample_pressure == 0 ||(ABS(last_sample_pressure - int(baro.paSample_)) < 10))) //only accept valid pressure readings
						{
							if (debug_level > 1) {
								Serial.printf("Updating Kalman filter with height %f and average accel %f \r\n", baro.zCmSample_, zAccelAverage);
								Serial.printf("baro sample pressure: %d last_sample_pressure %d\r\n", int(baro.paSample_), last_sample_pressure);
							}
							kf.Update(baro.zCmSample_, zAccelAverage, baroTimeDeltaSecs, &kfAltitudeCm, &kfClimbrateCps);
							//calculate the integrated cps over period of net_vario secs default 1s - represents 200 samples at 20ms sample rate
							avgeCps = runningAverageofCps(kfClimbrateCps);
							//Serial.printf("1s average cps %d\r\n", int(avgeCps));

							current_pressure = (int)baro.paSample_;
							current_altitude = (int)(kfAltitudeCm / 100.0);
							current_vario = (int)kfClimbrateCps;
							current_temperature = (int)baro.celsiusSample_;
							current_battery = batteryVoltage();
						}
					//	else
					//	{
					//		Serial.println("");
					//		Serial.printf("Ignoring sample with baro pressure of %d and last sample pressure of %d \r\n", int(baro.paSample_), last_sample_pressure);
					//		Serial.println("");

					//	}

						last_sample_pressure = (int)baro.paSample_;
						zAccelAccumulator = 0.0f;
						baroTimeDeltaSecs = 0.0f;
						...
						//Serial.println("tofb " + String( turn_on_off_buzzer) + "*");
						audio.VarioBeep(audioCps, turnonBuzzer);
			...
			if (drdyCounter >= 100) {
				...
				turnonBuzzer++;
				turnonBuzzer = (turnonBuzzer % 10);
#ifdef IMU_DEBUG

                           uint32_t elapsedUs = micros() - timePreviousUs; // calculate time  taken in reading and processing the data, should be less than 5mS worst case
