...
// Пины
#define BUTTON_PIN  4   // 7 кнопка (INPUT_PULLUP)
#define BUZZER_PIN  3   // 8 пьезоизлучатель (PWM)
...
void setup() {
  ...
  Wire.begin();
  if (!pressens.begin_I2C()) {
    Serial.println("DPS310 не найден! Проверьте подключение.");
    while (1) delay(10);
  }
  pressens.configurePressure(DPS310_64HZ, DPS310_64SAMPLES);
  pressens.configureTemperature(DPS310_64HZ, DPS310_64SAMPLES);
  // pressens.enablePressure(true);
  // pressens.enableTemperature(true);
...
float readAltitude() {
  float pressure = 0;
  if (!pressens.readPressure(pressure)) {
